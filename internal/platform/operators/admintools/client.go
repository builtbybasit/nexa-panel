...
func NewUnixClient(socketPath, tokenPath string) *UnixClient {
	...
	return &UnixClient{tokenPath: tokenPath, client: &http.Client{Transport: transport, Timeout: 10 * time.Minute}}
}
...
func (c *UnixClient) call(ctx context.Context, method, path string, input, output any) error {
	...
	if err != nil {
		return fmt.Errorf("call admin tool agent: %w", err)
	}
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		...
		if payload.Message == "" {
			payload.Message = "node agent rejected admin tool operation"
		}
