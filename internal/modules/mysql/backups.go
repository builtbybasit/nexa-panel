...
func (m *Module) CreateBackup(ctx context.Context, databaseID string, actor *string) (RestorePoint, jobs.Job, error) {
	...
	now := m.now().UTC()
	path := filepath.Join(m.backupRoot, database.EngineID, database.Name, id+".sql")
	...
	job, err := m.submitPlan(ctx, resourceRestorePoint, id, mysqloperator.ActionCreateBackup, actor)
	if err != nil {
...
func (m *Module) PrepareRestore(ctx context.Context, restorePointID string, actor *string) (RestorePoint, jobs.Job, error) {
	...
	job, err := m.submitPlan(ctx, resourceRestorePoint, model.ID, mysqloperator.ActionRestoreBackup, actor)
	if err != nil {
		return model.toRestorePoint(), jobs.Job{}, err
	}
