...
func TestSizeColumnsReachAnInstallCreatedBeforeThemExisted(t *testing.T) {
	...
	if err := persistence.Migrate(ctx, store, "mysql_databases", []string{schema}); err != nil {
		t.Fatal(err)
	}
	// Upgrading to the current binary.
	if err := persistence.Migrate(ctx, store, "mysql_databases", []string{schema, databaseSizeSchema}); err != nil {
		t.Fatal(err)
	}

	var count int
	if err := store.QueryRowContext(ctx, "SELECT COUNT(*) FROM pragma_table_info('mysql_databases') WHERE name IN ('size_bytes', 'size_observed_at')").Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 2 {
