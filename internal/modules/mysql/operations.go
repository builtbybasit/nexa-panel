...
func (m *Module) ApplyPlan(ctx context.Context, resourceType, resourceID string, actor *string) (jobs.Job, error) {
	...
	if err != nil {
		return jobs.Job{}, errors.New("a reviewed MySQL-family plan is not ready")
	}
	if m.now().UTC().After(plan.ExpiresAt) {
		return jobs.Job{}, errors.New("MySQL-family plan has expired and must be refreshed")
	}
	...
	if plan.Operation == mysqloperator.ActionCreateBackup {
		applyStatus = StatusBackingUp
	} else if plan.Operation == mysqloperator.ActionRestoreBackup {
...
func (m *Module) planJob(ctx context.Context, raw json.RawMessage, report func(int, string) error) (any, error) {
	...
	if json.Unmarshal(raw, &request) != nil || request.ResourceID == "" {
		return nil, errors.New("invalid MySQL-family planning request")
	}
	if err := report(20, "Loading managed MySQL-family desired state."); err != nil {
		return nil, err
	}
	change, err := m.changeFor(ctx, request.ResourceType, request.ResourceID, mysqloperator.Action(request.Action))
	if err != nil {
		m.failResource(context.WithoutCancel(ctx), request.ResourceType, request.ResourceID, err)
		return nil, err
	}
	if err := report(50, "Requesting an agent-signed MySQL-family plan."); err != nil {
		return nil, err
	}
	...
	if err := report(95, "MySQL-family plan is ready for approval."); err != nil {
		return nil, err
	}
