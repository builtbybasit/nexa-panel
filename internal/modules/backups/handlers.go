...
func (m *Module) getAccountHTTP(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, model.toAccount())
}
...
func (m *Module) deleteAccountHTTP(w http.ResponseWriter, r *http.Request) {
	if err := m.DeleteAccount(r.Context(), r.PathValue("id")); err != nil {
		if errors.Is(err, errAccountInUse) {
			writeError(w, http.StatusConflict, "backup_account_in_use", err.Error())
			return
		}
		writeError(w, http.StatusInternalServerError, "backup_account_delete_failed", "The backup account could not be removed.")
		return
	}
...
func (m *Module) testAccountHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, http.StatusBadGateway, "backup_test_failed", "The storage connection could not be tested.")
		return
	}
