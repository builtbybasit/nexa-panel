...
func (m *Module) failResource(ctx context.Context, resourceType, id string, failure error) {
	...
	if resourceType == resourceRole {
		model, err := m.getRoleModel(ctx, id)
		if err == nil && model.CredentialCiphertext != nil {
			_, _ = m.database.NewUpdate().Model((*roleModel)(nil)).Set("status = ?", StatusActive).Set("pending_credential_ciphertext = NULL").Set("pending_secret_digest = NULL").Set("failure = ?", message).Set("updated_at = ?", m.now().UTC()).Where("id = ?", id).Exec(ctx)
			return
		}
	}
...
func resourceTable(resourceType string) (string, error) {
	...
	case resourceInstance:
		return "postgresql_instances", nil
	case resourceRole:
		return "database_roles", nil
	case resourceDatabase:
		return "managed_databases", nil
	case resourceGrant:
		return "database_grants", nil
	case resourceRestorePoint:
		return "database_restore_points", nil
	default:
		return "", errors.New("PostgreSQL resource type is unsupported")
	}
}
