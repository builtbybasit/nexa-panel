...
/** `sizeBytes` is measured on read and absent until first measured; zero means an empty database, not an unknown one. `sizeObservedAt` says when it was last measured. */
export interface ManagedDatabase { id: string; instanceId: string; name: string; ownerRoleId: string; status: DatabaseStatus; sizeBytes?: number; sizeObservedAt?: string; lastJobId?: number; failure?: string; createdAt: string; updatedAt: string }
