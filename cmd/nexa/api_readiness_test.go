...
// TestAPIReadinessRequiresACurrentSchema keeps readiness honest about the state
// a half-completed self-update leaves behind: the database answers, but this
// binary's migrations have not been applied to it.
func TestAPIReadinessRequiresACurrentSchema(t *testing.T) {
	directory, err := os.MkdirTemp("/tmp", "nexa-ready-")
	if err != nil {
		t.Fatal(err)
	}
